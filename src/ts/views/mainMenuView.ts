import { translations, routing, settingsController, loadGameController } from "..";
...
export const mainMenuView = {
    ...
    buildFilledCard(slot: SaveSlotRecord, slotNum: number): string {
        ...
        const slotKey = slot.slotKey || ("slot-" + slotNum);
    ...
    bindFilledEvents() {
        ...
        $("#menu-saveSlotsGrid").on("click", ".slot-rename", function(e) {
            ...
            const newName = window.prompt(translations.text("renameSave") || "Rename save:", name);
        ...
        $("#menu-saveSlotsGrid").on("click", ".slot-delete", function(e) {
            ...
            const id = $(this).closest(".save-slot-card").data("id");
            const name = $(this).closest(".save-slot-card").find(".save-slot-name").text();
            if (window.confirm((translations.text("deleteSaveConfirm") || "Delete save?") + " " + name)) {
